...
data <- rbind(transaction, transaction_sup)
...
prod1 <- cast(prod1, form = tran_wk ~ vehicle, value = 'amount')

prod2 <- subset(ad_promotion, prod_assoc == 138936952)
prod2 <- cast(prod2, form = tran_wk ~ vehicle, value = 'amount')

prod3 <- subset(ad_promotion, prod_assoc == 138936953)
prod3 <- cast(prod3, form = tran_wk ~ vehicle, value = 'amount')

prodall <- subset(ad_promotion, prod_assoc == 'ALL')
prodall <- cast(prodall, form = tran_wk ~ vehicle, value = 'amount')
...
data_1 <- subset(data_1, select = -c(cust_id, store_id, prod_id, tran_prod_sale_amt, tran_prod_sale_qty, tran_prod_discount_amt, tran_prod_offer_cts, tran_prod_paid_amt, prod_unit_price, profit, discount_percent))
data_2 <- subset(data_2, select = -c(cust_id, store_id, prod_id, tran_prod_sale_amt, tran_prod_sale_qty, tran_prod_discount_amt, tran_prod_offer_cts, tran_prod_paid_amt, prod_unit_price, profit, discount_percent))
data_3 <- subset(data_3, select = -c(cust_id, store_id, prod_id, tran_prod_sale_amt, tran_prod_sale_qty, tran_prod_discount_amt, tran_prod_offer_cts, tran_prod_paid_amt, prod_unit_price, profit, discount_percent))
...
##Running test model
prod1_logit <- lm(logsales ~ weekly_price + weekly_discount + seas_index + holiday_ALLSAINTS + holiday_ASSUMPTION + holiday_CARNIVAL + holiday_CORPUS + holiday_EASTER +  holiday_LIBERTY + holiday_POPEVISIT + holiday_PORTUGAL + holiday_IMMACULATE + holiday_PrEASTER + holiday_PrLIBERTY + holiday_PrXMAS + holiday_XMAS + reachTV + reachRadio + Flyer + Email + paid_search + web_display, data= data_1)
summary(prod1_logit)
...
##To get the correlation values for each holiday
data1_mc <- sapply(data_1, as.numeric )
t <- cor(data1_mc)
corrplot(t, method="color")
t <- t[4:32, 4:32]

data2_mc <- sapply(data_2, as.numeric )
t2 <- cor(data2_mc)
...
# FINAL MODELS ----------------------------------------------------------------
# PRODUCT 1
prod1_logit <- lm(transformedsales ~ weekly_price + weekly_discount + seas_index + holiday_ALLSAINTS + holiday_ASSUMPTION + holiday_CARNIVAL + holiday_CORPUS + holiday_EASTER +  holiday_LIBERTY + holiday_POPEVISIT + holiday_PORTUGAL + holiday_IMMACULATE + holiday_PrEASTER + holiday_PrLIBERTY + holiday_PrXMAS + holiday_XMAS + reachTV + reachRadio + Flyer + Email + paid_search + web_display, data= data_1)
prod1_mult <- lm(logsales ~ weekly_price + weekly_discount + seas_index + holiday_ALLSAINTS + holiday_ASSUMPTION + holiday_CARNIVAL + holiday_CORPUS + holiday_EASTER +  holiday_LIBERTY + holiday_POPEVISIT + holiday_PORTUGAL + holiday_IMMACULATE + holiday_PrEASTER + holiday_PrLIBERTY + holiday_PrXMAS + holiday_XMAS + reachTV + reachRadio + Flyer + Email + paid_search + web_display, data= data_1)

# PRODUCT 2
prod2_logit <- lm(transformedsales ~ weekly_price + weekly_discount + seas_index + holiday_ALLSAINTS + holiday_ASSUMPTION + holiday_CARNIVAL + holiday_CORPUS + holiday_EASTER +  holiday_LIBERTY + holiday_POPEVISIT + holiday_PORTUGAL + holiday_IMMACULATE + holiday_PrEASTER + holiday_PrLIBERTY + holiday_PrXMAS + holiday_XMAS + reachTV + reachRadio + Flyer + store_display + Email + paid_search + web_display, data= data_2)
prod2_mult <- lm(logsales ~ weekly_price + weekly_discount + seas_index + holiday_ALLSAINTS + holiday_ASSUMPTION + holiday_CARNIVAL + holiday_CORPUS + holiday_EASTER +  holiday_LIBERTY + holiday_POPEVISIT + holiday_PORTUGAL + holiday_IMMACULATE + holiday_PrEASTER + holiday_PrLIBERTY + holiday_PrXMAS + holiday_XMAS + reachTV + reachRadio + Flyer + store_display + Email + paid_search + web_display, data= data_2)
...
## PRODUCT 2
data_2 <- data_2 %>%
  mutate(yhatm = exp(fitted(prod2_mult)), y = sales, p = predictors, n = nrow(data_2),
           theoretical_max = unique(theo_max), yhatl = theoretical_max*exp(fitted(prod2_logit)) / (exp(fitted(prod2_logit)) + 1)) 
...
## PRODUCT 3
data_3 <- data_3 %>%
  mutate(yhatm = exp(fitted(prod3_mult)), y = sales, p = predictors, n = nrow(data_3),
         theoretical_max = unique(theo_max), yhatl = theoretical_max*exp(fitted(prod3_logit)) / (exp(fitted(prod3_logit)) + 1))   
...
data_1$base <- predict(prod1_logit, newdata = base)
data_1 <- data_1 %>%
...
price_dt <- price_dt %>%
  mutate(weekly_price = baseprice) 
data_1$price_dt <- predict(prod1_logit, newdata = price_dt)
data_1 <- data_1 %>%
  mutate(baseprice_dt = theo_max * exp(price_dt) / (exp(price_dt) + 1),
         price_dt = pred-baseprice_dt)
...
data_1$promo_dt <- predict(prod1_logit, newdata = promotion_dt)
data_1 <- data_1 %>%
...
for (media in c('Flyer', 'Email', 'web_display', 'store_display', 'paid_search', 'reachTV', 'reachRadio')) {
  ...
  data_1 <- data_1 %>%
    mutate(c(col_name) = predict(prod1_logit, newdata = media_dt)]
...
# PRODUCT 2
data_2$pred <- predict(prod2_logit, newdata = data_2)
data_2 <- data_2 %>%
  mutate(pred = theo_max*exp(pred)/(exp(pred)+1))
...
base <- copy(data_2)
predicbase <- base %>%
  mutate(weekly_price = baseprice) tion_base
base[, c('weekly_discount', 'Email', 'Flyer', 'paid_search', 'web_display', 'store_display', 'reachTV', 'reachRadio')] <- 0
data_2$base <- predict(prod2_logit, newdata = base)
data_2 <- data_2 %>%
  mutate(base = theo_max * exp(base) / (exp(base) + 1))
...
price_dt <- price_dt %>%
  mutate(weekly_price = baseprice) 
data_2$price_dt <- predict(prod2_logit, newdata = price_dt)
data_2 <- data_2 %>%
  mutate(baseprice_dt = theo_max * exp(due_to_base_price) / (exp(due_to_base_price) + 1),
         price_dt = pred-baseprice_dt)
...
promotion_dt <- promotion_dt %>%
  mutate(weekly_discount = 0)
data_2$promo_dt <- predict(prod2_logit, newdata = promotion_dt)
data_2 <- data_2 %>%
  mutate(promo_dt = theo_max * exp(promo_dt) / (exp(promo_dt) + 1),
         promo_dt = pred-promo_dt)
...
for (media in c("Flyer", 'Email', 'web_display', 'store_display', 'paid_search', 'reachTV', 'reachRadio')) {
  ...
  col_name = paste('due_to', media, sep = '_')
  data_2 <- data_2 %>%
    mutate(c(col_name) = predict(prod2_logit, newdata = media_dt)]
...
# PRODUCT 3
data_3$pred <- predict(prod3_logit, newdata = data_3)
data_3 <- data_3 %>%
  mutate(pred = theo_max*exp(pred)/(exp(pred)+1))
...
predicbase <- base %>%
  mutate(weekly_price = baseprice) tion_base
base[, c('weekly_discount', 'Email', 'Flyer', 'paid_search', 'web_display', 'store_display', 'reachTV', 'reachRadio')] <- 0
data_3$base <- predict(prod3_logit, newdata = base)
data_3 <- data_3 %>%
  mutate(base = theo_max * exp(base) / (exp(base) + 1))
...
price_dt <- price_dt %>%
  mutate(weekly_price = baseprice) 
data_3$price_dt <- predict(prod3_logit, newdata = price_dt)
data_3 <- data_3 %>%
  mutate(baseprice_dt = theo_max * exp(due_to_base_price) / (exp(due_to_base_price) + 1),
         price_dt = pred-baseprice_dt)
...
promotion_dt <- promotion_dt %>%
  mutate(weekly_discount = 0)
data_3$promo_dt <- predict(prod3_logit, newdata = promotion_dt)
data_3 <- data_3 %>%
  mutate(promo_dt = theo_max * exp(promo_dt) / (exp(promo_dt) + 1),
         promo_dt = pred-promo_dt)
...
for (media in c('Flyer', 'Email', 'web_display', 'store_display', 'paid_search', 'reachTV', 'reachRadio')) {
  ...
  col_name = paste('due_to', media, sep = '_')
  data_3 <- data_3 %>%
    mutate(c(col_name) = predict(prod3_logit, newdata = media_dt)]
